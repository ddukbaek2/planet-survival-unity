...
public class BuffIconBar : MonoBehaviour {
    ...
    private void Update() {
        ...
        for (var index = 0; index < iconWeapons.Count; index++) {
            var weaponLevel = weaponManager.GetWeaponLevel(iconWeapons[index]);
            levelLabels[index].text = "Lv" + weaponLevel;
        }
    }

    private void Rebuild(List<WeaponType> ownedWeapons) {
        ...
        for (var index = 0; index < ownedWeapons.Count; index++) {
            ...
            var iconObject = new GameObject("Buff_" + weaponType, typeof(Image));
            ...
            var charObject = new GameObject("Char", typeof(TextMeshProUGUI));
            charObject.transform.SetParent(iconObject.transform, false);
            var charLabel = charObject.GetComponent<TextMeshProUGUI>();
            ...
            var charRect = charObject.GetComponent<RectTransform>();
            ...
            var levelLabel = levelObject.GetComponent<TextMeshProUGUI>();
